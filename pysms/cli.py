...
class ChatUI:
    ...
    def handle_incoming_message(self, sms: SMS):
        ...
        if (
            sms.sender == self.phone_number
            and hasattr(self.sms_handler, "debug")
            and self.sms_handler.debug
        ):
            print("[DEBUG] ✓ Message from target number!")
...
def main(
    ...
    port: str = typer.Option(
        "/dev/ttyUSB2", "--port", "-p", help="Serial port for GSM modem"
    ),
    ...
    try:
        ...
        if debug:
            typer.secho(
                "  🐛 [DEBUG MODE ENABLED - Will show detailed SMS processing info]",
        ...
        typer.secho(
            f"❌ Failed to initialize SMS handler: {e}", err=True, fg=typer.colors.RED
        )
        raise typer.Exit(1)
    ...
    try:
        while True:
            try:
                ...
                if user_input == "/multi":
                    typer.echo(
                        "📝 Multi-line mode: Type your message, end with a line containing only '.' to send"
                    )
                    lines = []
                    typer.echo("| ", nl=False)
