...
class SMSHandler:
    ...
    def _parse_sms_list(self, response: str) -> List[SMS]:
        ...
        while i < len(lines):
            ...
            if line.startswith("+CMGL:"):
                ...
                match = re.search(r'\+CMGL:\s*(\d+),\s*"([^"]*)",\s*"([^"]*)",\s*"([^"]*)"', line)
                if match:
                    sms = SMS(
    ...
    def _handle_cmt_message(self, line: str, callback: Callable[[SMS], None]) -> None:
        ...
        while time.time() - start_time < 2.0:
            if self.serial_conn.in_waiting:
                try:
                    ...
                    if (msg_line.startswith(("+CMT:", "+CMTI:", "OK", "ERROR", "AT+")) or
                        (not msg_line and message_lines)):
                        if self.debug:
                            print(f"[DEBUG] End of message detected")
                        break
        ...
        if message_lines:
            ...
            if self.debug:
                print(f"[DEBUG] No message content found for CMT")
    ...
    def _handle_cmti_message(self, line: str, callback: Callable[[SMS], None]) -> None:
        ...
        try:
            ...
            if self.debug:
                print(f"[DEBUG] Retrieved SMS: from={sms.sender}, message={sms.message}")
            callback(sms)
        except Exception as e:
    ...
    def _read_sms_by_index(self, index: int) -> SMS:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("+CMGR:"):
                ...
                match = re.search(r'\+CMGR:\s*"([^"]*)",\s*"([^"]*)",\s*"([^"]*)"', line)
                if match:
                    sms = SMS(
                        index=index,
